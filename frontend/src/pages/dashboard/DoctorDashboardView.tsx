                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={callingNext || queueEntries.filter(e => e.status === 'waiting' && typeof e.id === 'number').length === 0}
                      className="bg-teal-500 hover:bg-teal-600 disabled:opacity-60 text-white font-bold py-3 px-6 rounded-full transition duration-300"
                    >
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {queueEntries.length === 0 ? (
                            ...
                            queueEntries.map((entry) => (
                              <tr key={entry.id} className={`hover:bg-gray-50 ${entry.status === 'in_consultation' ? 'bg-teal-50' : ''}`}>
                                <td className="px-6 py-4 text-sm font-medium text-gray-900">{entry.queue_number ?? '-'}</td>
                                <td className="px-6 py-4 text-sm text-gray-600">
                                  {entry.patient ? `${entry.patient.first_name} ${entry.patient.last_name}` : 'Unknown'}
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                    ...
                                    entry.status === 'scheduled' ? 'bg-purple-100 text-purple-800' :
                                    'bg-gray-100 text-gray-800'
                                  }`}>
                                ...
                                <td className="px-6 py-4 text-sm text-gray-600">
                                  {entry.checked_in_at ? new Date(entry.checked_in_at).toLocaleTimeString() : (entry.appointment?.appointment_time || '-')}
                                </td>
                                <td className="px-6 py-4 text-sm">
                                  ...
                                  {entry.status === 'in_consultation' && typeof entry.id === 'number' && (
                                    <button
                                  ...
                                  {(entry.status === 'waiting' || entry.status === 'in_consultation') && typeof entry.id === 'number' && (
                                    <button
                                      onClick={() => updateQueueStatus(entry.id as number, 'cancelled')}
