...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (authToken) {
      ...
      const authUser = (authUserString && authUserString !== 'undefined') ? JSON.parse(authUserString) : null;
      const rolesArray: string[] = authUser?.roles || [];
      const role = rolesArray.length > 0 ? rolesArray[0].toLowerCase() : '';
      if (role) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user) localStorage.setItem('authUser', JSON.stringify(response.user));
    ...
    <div className="relative min-h-screen bg-center bg-cover" style={{ backgroundImage: "url('/images/Hero.png')" }}>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4">
        <div className="w-full max-w-md">
          <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="mb-8 text-center">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.3 }} className="p-8 space-y-6 border border-gray-200 shadow-2xl bg-white/95 backdrop-blur-lg rounded-2xl">
            ...
            {error && <div className="px-4 py-3 text-sm text-red-800 border border-red-200 rounded-lg bg-red-50">{error}</div>}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button type="submit" disabled={loading} className="w-full px-8 py-3 font-bold text-white transition duration-300 bg-teal-600 rounded-full shadow-lg hover:bg-teal-700 disabled:opacity-60 disabled:cursor-not-allowed hover:shadow-xl">
